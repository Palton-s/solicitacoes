...
// Logs iniciais
error_log("========== PROCESSAR-SUSPENSAO.PHP INICIADO ==========");
error_log("Processar Suspensão - REQUEST_METHOD: " . $_SERVER['REQUEST_METHOD']);
error_log("Processar Suspensão - POST dados: " . print_r($_POST, true));

// Validar que é uma requisição POST
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    error_log("Processar Suspensão - ERRO: Não é POST");
...
try {
    require_sesskey();
    error_log("Processar Suspensão - Sesskey válido");
} catch (Exception $e) {
    error_log("Processar Suspensão - ERRO: Sesskey inválido - " . $e->getMessage());
    redirect(new moodle_url('/local/solicitacoes/solicitar-suspensao.php'),
...
if (optional_param('cancel', 0, PARAM_BOOL)) {
    error_log("Processar Suspensão - Usuário cancelou");
...
// Debug log
error_log("Processar Suspensão - Dados recebidos:");
...
if (empty($curso_id) || $curso_id <= 0) {
    ...
    error_log("Processar Suspensão - ERRO: Curso vazio ou inválido");
}

if (empty(trim($usuarios_ids))) {
    $errors[] = get_string('error_usuarios_required', 'local_solicitacoes');
    error_log("Processar Suspensão - ERRO: Usuários vazio");
}

if (empty(trim($observacoes))) {
    $errors[] = get_string('error_motivo_required', 'local_solicitacoes');
    error_log("Processar Suspensão - ERRO: Motivo vazio");
}

error_log("Processar Suspensão - Total de erros de validação: " . count($errors));

// Se houver erros, redirecionar de volta
if (!empty($errors)) {
    $error_message = implode('<br>', $errors);
    error_log("Processar Suspensão - Redirecionando com erros: " . $error_message);
    redirect(new moodle_url('/local/solicitacoes/solicitar-suspensao.php'),
...
error_log("Processar Suspensão - Sem erros, processando solicitação...");
$data = new \stdClass();
...
error_log("Processar Suspensão - Chamando controller...");
$success = \local_solicitacoes\solicitacoes_controller::process_request_submission($data);
error_log("Processar Suspensão - Resultado do controller: " . ($success ? 'SUCESSO' : 'FALHA'));

if ($success) {
    error_log("Processar Suspensão - Redirecionando para confirmacao.php");
    redirect(new moodle_url('/local/solicitacoes/confirmacao.php'),
             get_string('success_submit', 'local_solicitacoes'), 
             null, 
             \core\output\notification::NOTIFY_SUCCESS);
} else {
    error_log("Processar Suspensão - Redirecionando de volta com erro");
    redirect(new moodle_url('/local/solicitacoes/solicitar-suspensao.php'),
